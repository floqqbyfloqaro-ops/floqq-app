...
--
-- The role can't be handed to someone else: anyone getting off earlier can't pay the full fare.
-- A payer who can't pay cancels their seat instead; the group is rescored and whoever now has the
-- last stop becomes the payer. Payout setup is not a deadline before the ride (agreed "pay now,
-- set up later"): FLOQQ keeps the reimbursement until the payer finishes Stripe's onboarding.
--
-- Safe to run more than once.

-- 1. The payer, per group. Null until the group is confirmed (and payments are on).
...
-- Leftovers of a first draft that let the payer hand the role to another passenger.
alter table public.taxi_groups
  drop column if exists payer_status,
  drop column if exists payer_declined_request_ids;
drop function if exists public.passenger_decline_payer_role(uuid, uuid);
drop function if exists public.passenger_volunteer_as_payer(uuid, uuid);
...
-- 4. Keeps a confirmed group's payer valid. A payer who is still a member is kept (locked).
-- Otherwise (first run after confirmation, or the payer left the group) the member with the last
-- stop becomes the payer. Service role only (payments-sync-holds). Returns the new payer, if any,
...
begin
  ...
  if v_group.payer_request_id is not null and exists (
  ...
  if not found then
    return jsonb_build_object('changed', false, 'reason', 'no_members');
  end if;

  update public.taxi_groups
  set payer_request_id = v_payer.id, payer_user_id = v_payer.user_id,
  ...
  values (p_group_id, v_payer.id, 'payer_assigned',
          jsonb_build_object('reason', case when v_group.payer_request_id is null and v_group.payer_assigned_at is null
                                            then 'last_stop' else 'payer_left_group' end),
          'system');

  return jsonb_build_object('changed', true, 'request_id', v_payer.id, 'user_id', v_payer.user_id);
end;
$$;
